...
class ImdbExtractor(object):
    ...
    def doc_generator(self, imdb_dir, dataset, include_label=False):
        ...
        for d, label in dirs:
            for filename in os.listdir(d):
                # with tf.gfile.Open(os.path.join(d, filename)) as imdb_f:
                with open(os.path.join(d, filename), 'r') as imdb_f:
                    doc = imdb_f.read().strip()
                    if include_label:
                        yield doc, label
                    else:
    ...
    def generate_samples(self, data_dir, tmp_dir, dataset_split):
        ...
        # train = dataset_split == problem.DatasetSplit.TRAIN
        # dataset = "train" if train else "test"
        dataset = "train" if dataset_split == 'train' else 'test'
        for doc, label in self.doc_generator(imdb_dir, dataset, include_label=True):
            tok_doc = ' '.join(encode(native_to_unicode(doc))).strip()
            tok_doc = tok_doc.replace('\t', ' ')
            tok_doc = re.sub(' +', ' ', tok_doc)
            tok_doc = tok_doc.strip()

            if "\t" in tok_doc:
                raise ValueError(f'has tab token: {tok_doc}')
            yield {
                "inputs": tok_doc,
                "label": int(label),
            }

    def convert(self, output_file, generator):
        print(f'Convert {output_file}')
        with open(output_file, 'w') as f:
            for i, example in enumerate(generator):
                if i % 500 == 0:
                    print(f'example [{i}][{example["label"]}]: {example["inputs"][:15]}')
                f.write(f'{example["inputs"]}\t{example["label"]}\n')


if __name__ == '__main__':
    folder = os.environ['imdb_dir']
    assert os.path.exists(folder)

    extractor = ImdbExtractor(folder)

    train_out = os.path.join(folder, 'train.tsv')
    dev_out = os.path.join(folder, 'dev.tsv')

    extractor.convert(train_out, extractor.generate_samples('', folder, 'train'))
    extractor.convert(dev_out, extractor.generate_samples('', folder, 'dev'))
